...
def read_graphfile(f):
    ...
    weights = np.hstack([graph_data[:, 2], graph_data[:,2]])
    max_nid = np.max(row)
    return csr_matrix((weights, (row,col)), shape=(max_nid+1,max_nid+1))

def read_labelfile(f):
    label_data = np.genfromtxt(f, delimiter=' ', dtype=float)
