...
class Base(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self,graph,max_iter=40):
    ...
    def fit(self,x,y):
        """Fit a graph-based semi-supervised l
...
class LGC(Base):
    ...
    def __init__(self,graph=None,alpha=0.99,max_iter=40):
        super(LGC, self).__init__(graph,max_iter=40)
        self.alpha=alpha
    ...
    def _build_base_matrix(self):
        ...
        #B = self.y_
        s = [0] * n_samples
        B = csr_matrix((self.y_, (range(n_samples), s)), shape = (n_samples, 1), dtype = float)
